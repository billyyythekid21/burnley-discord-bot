...
class Level(commands.Cog):
    ...
    def level_up(self, author_id):
        ...
        if current_experience >= math.ceil((6 * (current_level ** 4)) / 3):
    ...
    async def on_message(self, message):
        ...
        if self.level_up(author_id):
            level_up_embed = discord.Embed(title="Level Up!!!!", colour=discord.Colour.green())
            level_up_embed.add_field(name="Congratulations!", value=f"{message.author.mention} has just levelled up to Level {self.users[author_id]["Level"]}!")

        await message.channel.send(embed=level_up_embed)
    
    @commands.hybrid_command(name="level", description="Check your user level.", with_app_command = True, aliases=["rank", "lvl"])
    async def level(self, ctx, user: discord.User=None):
        ...
        level_card = discord.Embed(title=f"{user.name}'s level + XP", colour=discord.Colour.random())
